...
import { MapPin, Leaf, Building2, Landmark, Info, Search, Navigation, AlertTriangle, Crosshair, Home, Palette } from 'lucide-react';
import { getPlaces, getHomestays, getProducts } from '../services/api';
...
const STATIC_PLACES = [
  ...
  { _id: '6', name: 'Sarhul Festival Ground', category: 'Cultural', type: 'place', description: 'The main tribal festival of the Oraon and Munda tribes, celebrating spring.', images: ['https://images.unsplash.com/photo-1602079428437-b38b4f2e97a6?w=800'], coordinates: { lat: 23.34, lng: 85.31 } },
  ...
  { _id: 'homestay1', name: 'Ranchi Village Retreat', category: 'Homestays', type: 'homestay', description: 'Experience genuine rural lifestyle with modern comforts near Ranchi.', images: ['https://images.unsplash.com/photo-1542718610-a1d656d1884c?w=800'], coordinates: { lat: 23.35, lng: 85.30 } },
  { _id: 'product1', name: 'Sohrai Art Workshop', category: 'Handicrafts', type: 'handicraft', description: 'Authentic tribal mural paintings by local Hazaribagh artisans.', images: ['https://images.unsplash.com/photo-1558000143-a6125028dedf?w=800'], coordinates: { lat: 23.99, lng: 85.36 } },
];
...
export default function MapPage() {
  ...
  useEffect(() => {
    Promise.all([
    ...
    ]).then(([placesRes, homestaysRes, productsRes]) => {
      let combined = [];
      ...
      // Format Homestays
      if (homestaysRes.data?.length > 0) {
        const hData = homestaysRes.data.map(h => ({
          ...h, type: 'homestay', category: 'Homestays'
        }));
        combined = [...combined, ...hData];
      }

      // Format Products (Handicrafts)
      if (productsRes.data?.length > 0) {
        const prData = productsRes.data.map(pr => ({
          ...pr, type: 'handicraft', category: 'Handicrafts'
        }));
        combined = [...combined, ...prData];
      }
  ...
  const filteredPlaces = places.filter(p => {
    ...
    if (filter === 'Places' && ['Homestays', 'Handicrafts'].includes(p.category)) return false;
  ...
  return (
    <div className="min-h-screen bg-light flex flex-col relative overflow-hidden">
      ...
      {/* Header */}
      <div className="bg-gradient-to-r from-primary to-green-600 text-white py-14 px-4 text-center shrink-0">
      ...
      <div className="max-w-7xl mx-auto px-4 py-8 w-full flex-grow flex flex-col min-h-0">
        ...
        <div className="flex flex-col md:flex-row gap-4 justify-between items-center mb-6">
          <div className="flex gap-2 flex-wrap justify-center max-w-2xl">
            {['All', 'Places', 'Homestays', 'Handicrafts', 'Eco', 'Heritage', 'Cultural'].map((cat) => (
