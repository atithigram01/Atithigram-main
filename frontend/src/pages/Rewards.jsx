...
const ACTIONS = [
  { icon: <Leaf size={22} className="text-green-600" />,   action: 'Book a Verified Homestay',    points: 20, bg: 'bg-green-50'  },
  { icon: <Star size={22} className="text-yellow-600" />,  action: 'Buy from Local Artisan',       points: 10, bg: 'bg-yellow-50' },
  { icon: <Trophy size={22} className="text-purple-600" />,action: 'Write a Travel Review',        points: 5,  bg: 'bg-purple-50' },
  { icon: <Gift size={22} className="text-blue-600" />,    action: 'Refer a Friend',               points: 15, bg: 'bg-blue-50'  },
];

const REWARDS = [
  { title: '10% Off Homestay',    cost: 50,  icon: '🏡' },
  { title: 'Free Guided Tour',    cost: 100, icon: '🗺️' },
  { title: 'Exclusive Artisan Gift', cost: 80, icon: '🎁' },
  { title: 'Priority Booking',     cost: 30,  icon: '⚡'  },
];
      ...
      <div className="max-w-5xl mx-auto px-4 py-14">
        ...
        <div className="mt-16 bg-gradient-to-r from-primary to-green-700 rounded-2xl p-8 text-white text-center">
          ...
          <div className="grid grid-cols-3 gap-4 max-w-lg mx-auto">
            {[
              { val: `${Math.floor(points / 20)}`, label: 'Families Supported' },
              { val: `${Math.floor(points / 10)}`, label: 'Artisans Helped' },
